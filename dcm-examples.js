...
var examples = [
  {
    ...
    name: 'Dragon curve',
    bg: 'dragon color.png',
    description: 'the dragon curve placeholder',
  ...
  },
  {
    ...
    name: 'Barsley Fern',
    bg: 'barnsley.gif',
    description: 'the fern placeholder',
  ...
  },
  {
    ...
    name: 'Hexaflake',
    bg: 'hexaflake.png',
    description: 'the asdf placeholder',
    inPoints: [ {x: 241, y: 276}, {x: 92, y: 189}, {x: 390, y: 190} ],
    outPoints: [
      [ {x: 92, y: 190}, {x: 42, y: 161}, {x: 142, y: 161} ], 
...
function unhighlightThen(callback) {
  setTimeout(function() {
    highlightPoint();
...
function stepOne(exObj, inPInd) {
  ...
  unhighlightThen(function(){stepOne(exObj, inPInd+1);});
}
...
function stepTwo(exObj, outPInd, outPSubInd) {
  ...
  if(outPSubInd == exObj.outPoints[outPInd].length - 1) {
    unhighlightThen(function(){stepTwo(exObj, outPInd+1, 0);});
  }
  else {
    unhighlightThen(function(){stepTwo(exObj, outPInd, outPSubInd+1);});
  }
...
function stepThree(exObj) {
  ...
  setTimeout(function(){stepFour(exObj);}, TIMEPERSTEP*2);
}
